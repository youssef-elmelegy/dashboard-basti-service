...
const editSweetSchema = z.object({
  id: z.string(),
  name: z
    .string()
...
type EditSweetFormValues = z.infer<typeof editSweetSchema>;

interface EditSweetFormProps {
  sweet: Sweet;
  onSubmit: (values: EditSweetFormValues) => void;
  isLoading?: boolean;
}

export function EditSweetForm({
  sweet,
  onSubmit,
  isLoading = false,
}: EditSweetFormProps) {
  const [imagePreview, setImagePreview] = useState<string>(sweet.image);

  const form = useForm<EditSweetFormValues>({
    resolver: zodResolver(editSweetSchema),
    defaultValues: {
      id: sweet.id,
      name: sweet.name,
