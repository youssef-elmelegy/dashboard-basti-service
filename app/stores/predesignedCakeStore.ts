...
export const usePredesignedCakeStore = create<PredesignedCakeState>((set) => ({
  predesignedCakes: [],
  isLoading: false,
  error: null,
  pagination: {
  ...
  updatePredesignedCake: async (id: string, data: UpdatePredesignedCakeDto) => {
    set({ isLoading: true, error: null });
    try {
  ...
  togglePredesignedCakeActive: async (id: string) => {
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        set({
          error: response.message || "Failed to toggle predesigned cake status",
          isLoading: false,
        });
        return false;
      }
